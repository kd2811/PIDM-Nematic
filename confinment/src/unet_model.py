...
class Residual(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        return self.fqn(x, *args, **kwargs) + x
...
class SpatialLinearAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # chunk 将 channel （c=3*hidden_dim） 分为3部分
        qkv = self.to_qkv(x).chunk(3, dim = 1)
        q, k, v = rearrange_many(qkv, 'b (h c) x y -> b h c (x y)', h = self.heads)  # 将channel分为h个头 x,y is height and width
        # final shape is b*f, head, channel, h*w

        q = q.softmax(dim = -2)  #softmax along channel 
        k = k.softmax(dim = -1) #softmax along height and width

        q = q * self.scale
        v = v / (h * w) # added this (not included in original repo)

        context = torch.einsum('b h d n, b h e n -> b h d e', k, v)
        # final context shape is b*f, head, channel_k, channel_v

        out = torch.einsum('b h d e, b h d n -> b h e n', context, q)
        # final out shape is b*f, head, channel_v, h*w

        out = rearrange(out, 'b h c (x y) -> b (h c) x y', h = self.heads, x = h, y = w)
        # final out shape is b*f, head*channel_v, h, w
        out = self.to_out(out)
        return rearrange(out, '(b f) c h w -> b c f h w', b = b)
        # return batch, channel, frame, height, wdith

# attention along space and time
class EinopsToAndFrom(nn.Module):
    def __init__(self, from_einops, to_einops, fn):
        super().__init__()
        self.from_einops = from_einops # 原始维度布局，如 'b c f h w'
        self.to_einops = to_einops # 目标维度布局，如 'b (h w) f c'
        self.fn = fn   # 需要适配的模块（如Attention）

    def forward(self, x, **kwargs):
        shape = x.shape # 记录原始形状，如 (B, C, F, H, W)
        reconstitute_kwargs = dict(tuple(zip(self.from_einops.split(' '), shape)))  # 提取维度值：{'b': B, 'c': C, 'f': F, 'h': H, 'w': W}
...
class Unet3D(nn.Module):
    def __init__(
        ...
        # generate a function input is dim, EinopsToAndFrom can 
        # change the shpae of input feature from 'b c f h w', 'b (h w) f c'
        # Attention layer
        ...
        # initial conv
        init_dim = default(init_dim, dim) # init_dim = dim if init_dim is None else init_dim
        ...
        # after init_conv, shape is [B, init_dim, F=1, H, W]
        
        self.init_temporal_attn = Residual(PreNorm(init_dim, temporal_attn(init_dim)))
        # x --> norm layer(x) --> temporal attention layer(norm(x)) --> output + x
        # dimensions
        dims = [init_dim, *map(lambda m: dim * m, dim_mults)]
        # dim_mults = (1,2,4,8) -> dims = [init_dim, dim, 2*dim, 4*dim, 8*dim]
        in_out = list(zip(dims[:-1], dims[1:]))
        # in_out = [(init_dim, dim), (dim, 2*dim), (2*dim, 4*dim), (4*dim, 8*dim)]
        # time conditioning
        ...
        # time_mlp: [B, C] --> [B, time_dim] (time_dim = 4*dim)
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            # dim_in, dim_out = dim, 2*dim
            # dim_in, dim_out = 2*dim, 4*dim
            # dim_in, dim_out = 4*dim, 8*dim
            ...
            self.downs.append(nn.ModuleList([
                ...
                # resnet --> resnet --> resiudl --> downsample
                # x --> norm layer(x) --> spatial attention layer(norm(x)) --> output + x
            ]))
        ...
        out_dim = default(out_dim, channels) # out_dim = channels if out_dim is None else out_dim
        self.final_conv = nn.Sequential(
            block_klass(dim * 2, dim),
            nn.Conv3d(dim, out_dim, 1)
        )
        # input of final_conv is [B, 2*dim, F, H, W]
        # after final_conv, shape is [B, channels, F, H, W]
        ...
        # Input cond shape is [B, C, H, W]  --> [B, init_dim, H, W]

        self.combine_conv = torch.nn.Conv2d(init_dim*2, init_dim, kernel_size=1, stride=1, padding=0)
        # input shape is [B, 2*init_dim, H, W] --> [B, init_dim, H, W]
...
##### NOTE:!!!!!!!!!!
    ...
    def forward(
        ...
        if exists(Mask_input):
            # Mask_input.shape = [B, H, W]
            # Mask_input = generalized_b_xy_c_to_image(Mask_input)
            Mask_input = Mask_input.unsqueeze(1)
            # Mask_input.shape = [B, 1, H, W]
            Mask_input = Mask_input.unsqueeze(1)
            # Mask_input.shape = [B, 2, 1, H, W]
            Mask_input = Mask_input.repeat(1, x.shape[1], 1, 1, 1)
            # Mask_input.shape = [B, C, 1, H, W]
            if Mask_input.shape != x.shape:
                print('Mask_input shape:', Mask_input.shape)
                print('x shape:', x.shape)
                raise ValueError('Mask_input shape must be same as x shape.')
            else: 
                x = x * Mask_input
        ...
        if exists(cond)&exists(Mask_input):
            batch, device = x.shape[0], x.device
            ...
            # cond.shape = [B, C, H, W]
            # cond = cond.repeat(1, Mask_input.shape[1], 1, 1)
            # cond.shape = [B, C, H, W]

            # Mask_input shape = [B, C, 1, H, W]
            # mask = 1 - Mask_input
            # mask = torch.ones_like(Mask_input)
            mask = Mask_input
            mask = mask.squeeze(2)
            # mask shape = [B, C, H, W]
            probability_mask = prob_mask_like(cond.shape, null_cond_prob, device)
            cond = cond * mask * probability_mask
            # cond shape = [B, C, H, W]
            cond_emb = self.emb_conv(cond)
            # Input cond shape is [B, C, H, W]  --> [B, init_dim, H, W]
            
            x = torch.cat((x.squeeze(2), cond_emb), dim=1) # concatenate to channel dimension
            # x.squeeze(2).shape = [B, init_dim, H, W]
            # cond_emb.shape = [B, init_dim, H, W]
            # x.shape = [B, 2*init_dim, H, W]

            x = self.combine_conv(x).unsqueeze(2)
            # x.shape = [B, init_dim, 1, H, W]

        r = x.clone()
        # r.shape = [B, init_dim, 1, H, W]  same as x
        ...
        x = self.final_conv(x)
        # after final_conv, shape is [B, channels, 1, H, W]
